...
#
# This version, dated Jun-2022, allows for lambda > 0.
#
#####################################################################################

#####################################################################################
# h(.) using the Adams scheme
hA.GB <- function(params)function(n,a,bigT){
  ...
  F <- function(u,h){ -0.5*u*(u+1i)+(1i*rho*nu*u-lam)*h+0.5*nu^2*h^2}
...
dhA.GB.raw <- function(params)function(n, a, bigT){
...
# Vectorize the above functions
dhA <- function(params)Vectorize(dhA.GB.raw(params),vectorize.args = "a")
hA <- function(params)Vectorize(hA.GB(params),vectorize.args = "a")
#####################################################################################
...
phiRoughHeston.Adams <- function (params, xiCurve, nSteps){
...
otmRoughHeston.raw <- function (params, xiCurve, nSteps) 
  function(k, t) {
    k.minus <- (k < 0) * k
    phi <- phiRoughHeston.Adams(params, xiCurve, n = nSteps)
...
# Vectorize the above function
otmRoughHeston.Adams <- function (params, xiCurve, nSteps){
...
#####################################################################################
impliedVolRoughHeston.Adams <- function (params, xiCurve, nSteps) 
  function(k, t) {
    ivOTM(1, exp(k), t, otmRoughHeston.Adams(params, xiCurve, nSteps)(k, t))
  }
#####################################################################################